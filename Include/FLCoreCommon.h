...
public:
	...
	std::map<uint, struct MarketGoodInfo> market_map;
};
...
public:
	uint iDunno1; // 0
	std::list<DamageEntry> damageentries; // 4
	bool bDestroyed; // 12
	uint iDunno2; // 16
...
namespace Universe
{
	...
	struct IMPORT ISystem
	{
		...
		std::vector<ISystem*> connections;
		uchar   visit;
		uint   strid_name;
		uint   ids_info;
		const char* file;			// CacheString
		class Vector NavMapPos;
		std::list<struct IZone*> zones;		// std::list
	...
	struct IMPORT IZone
	{
		...
		std::list<FactionSpawn> factionSpawns;
		//std::/*weighted_*/vector<FactionSpawn> factionSpawns;
...
namespace PhySys
{
	...
	{
		// The object ramming another object
		CObject* collider;
		// The object being rammed
		CObject* collidedWith;
		uint iDunno1[2];
		Vector position;
		Vector normal;
		Vector velocity;
	...
	IMPORT  int  FindRayIntersect(struct CObject *, class Vector const &, class Vector const &, RayHit *, int);
...
public:
	...
	void* custom_wrapper_class; // 0x18
	void* custom_wrapper_class2; // 0x1C
	char* mounted_hardpoint; // 0x20
};
...
IMPORT class Matrix   EulerMatrix3D(class Vector const &);
...
IMPORT class Matrix   GetDirectionMatrix3D(class Transform const &, class Vector const &);
...
IMPORT class Matrix   LookMatrix3D(class Vector const &);
IMPORT class Matrix   LookMatrix3DYup(class Vector const &);
...
IMPORT class Matrix   RotateMatrix3D(class Vector const &);
...
struct Camera
{
	...
	float fFovYHud;
	float fCosFovX;
	float fAlwaysZeroFovX;
	float fSinFovX;
	float fAlwaysZeroFovY;
	float fCosFovY;
	float fSinFovY;
	float fFovX;
	float fFovY;
	...
	float fZNear;
	float fZFar;
	uint iScreenExtents[4];
	uint iDunno;
	Watchable* watchable; // 49
	unsigned int iDunno3[6];
	Vector vDunno1;
	unsigned int iDunno4[6];
	union
	{
		float fCameraOffset[3]; // starts at 64
