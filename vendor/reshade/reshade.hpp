...
namespace reshade
{
    ...
    inline void log_message(log_level level, const char *message)
    {
        static const auto func =
            reinterpret_cast<void (*)(HMODULE, int, const char *)>(GetProcAddress(internal::get_reshade_module_handle(), "ReShadeLogMessage"));
    ...
    /// </summary>
    /// <param name="path">Pointer to a string buffer that is filled with the file path to the preset, or <see langword="nullptr"/> to query the necessary
    /// size.</param> <param name="path_size">Pointer to an integer that contains the size of the string buffer and is set to the actual length of the string,
    /// including the null-terminator.</param>
    inline void get_reshade_base_path(char *path, size_t *path_size)
    {
        static const auto func = reinterpret_cast<bool (*)(char *, size_t *)>(GetProcAddress(internal::get_reshade_module_handle(), "ReShadeGetBasePath"));
        func(path, path_size);
    }
    ...
    /// <param name="value_size">Pointer to an integer that contains the size of the string buffer and is set to the actual length of the string, including the
    /// null-terminator.</param> <returns><see langword="true"/> if the specified config value exists, <see cref="false"/> otherwise.</returns>
    ...
    inline bool register_addon(HMODULE addon_module, [[maybe_unused]] HMODULE reshade_module = nullptr)
    {
        ...
        const auto func = reinterpret_cast<bool (*)(HMODULE, uint32_t)>(GetProcAddress(reshade_module, "ReShadeRegisterAddon"));
    ...
    inline void unregister_addon(HMODULE addon_module, [[maybe_unused]] HMODULE reshade_module = nullptr)
    {
        ...
        const auto func = reinterpret_cast<bool (*)(HMODULE)>(GetProcAddress(reshade_module, "ReShadeUnregisterAddon"));
        if (func != nullptr)
        {
            func(addon_module);
        }
    ...
    /// <para>The callback function is then called whenever the application performs a task associated with this event (see also the <see cref="addon_event"/>
    /// enumeration).</para>
    /// </summary>
    ...
    {
        static const auto func =
            reinterpret_cast<void (*)(reshade::addon_event, void *)>(GetProcAddress(internal::get_reshade_module_handle(), "ReShadeRegisterEvent"));
        if (func != nullptr)
        {
    ...
    {
        static const auto func =
            reinterpret_cast<void (*)(reshade::addon_event, void *)>(GetProcAddress(internal::get_reshade_module_handle(), "ReShadeUnregisterEvent"));
        if (func != nullptr)
        {
            func(ev, static_cast<void *>(callback));
        }
