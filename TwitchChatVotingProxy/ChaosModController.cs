...
internal class ChaosModController
{
    ...
    private List<IVoteOption> _activeVoteOptions = new();
    ...
    {
        // Get total votes
        var votes = _activeVoteOptions.Select(_ => _config.RetainVotes ? _.Votes + 1 : _.Votes).ToList();
        var totalVotes = 0;
    ...
    private void OnGetVoteResult(object? sender, GetVoteResultArgs e)
    {
        ...
        e.ChosenOption = _config.VotingMode switch
        {
    ...
    private async void OnNewVote(object? sender, NewVoteArgs e)
    {
        _activeVoteOptions = e.VoteOptionNames.ToList().Select((voteOptionName, index) =>
        {
            ...
            var match = _voteCounter % 2 == 0
                ? _config.VotingPrefix + (index + 1)
                : _config.VotingPrefix + (index + 1 + _activeVoteOptions.Count);
        ...
        if (_config.VotingMode == VotingMode.Percentage)
        {
    ...
    private void OnVoteReceiverMessage(object? sender, MessageArgs e)
    {
        ...
        if (_config.PermittedUsernames?.Count > 0 && e.Username is not null)
        {
            var found = _config.PermittedUsernames.Any(name => name == e.Username);
