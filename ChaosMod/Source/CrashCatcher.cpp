...
{
    Log(StringUtils::wstos(std::vformat(templateStr, std::make_wformat_args(std::forward<Args&>(args)...))));
}

template <class... Args>
void LogInternal(const char* templateStr, Args&&... args)
{
    Log(std::vformat(templateStr, std::make_format_args(std::forward<Args&>(args)...)));
}

void __stdcall CrashCatcher::LogContent47bc4()
{
    Log("Exception/Crash in content.dll:0xCb_47bc4 - probably missing formation in faction_props.ini/formations.ini - exiting");
    Sleep(1000);
    std::exit(-1);
}
...
void CrashCatcher::CrashProc6F671A0(int arg1)
{
    ...
    {
        Log(std::format("Crash suppression in CrashProc6F671A0(arg1={:#X})", arg1));
    }
}
...
CObject* CrashCatcher::GetRoot(CObject* child)
{
    ...
    {
        Log(std::format("Crash suppression in GetRoot(child={})", child->get_archetype()->archId));
        return child;
    }
}
...
char __stdcall CrashCatcher::FixCommon6F8B330Detour(int arg1)
{
    ...
    {
        Log(std::format("Crash suppression in CrashProc6F8B330(arg1={:#X})", arg1));
    }
...
void __stdcall CrashCatcher::FixCommon6F78DD0Detour(int arg1, int arg2)
{
    ...
    {
        Log(std::format("Crash suppression in CrashProc6F78DD0(arg1={:#X}, arg2={:#X})", arg1, arg2));
    }
}
