...
IDirect3D9* Direct3D8::ProxyInterface;
IDirect3DDevice9* Direct3DDevice8::ProxyInterface;
...
Direct3DDevice8::Direct3DDevice8(Direct3D8* d3d, IDirect3DDevice9* ProxyInterface, BOOL EnableZBufferDiscarding)
    : D3D(d3d), ZBufferDiscarding(EnableZBufferDiscarding)
{
    ...
    UiManager::Setup(ProxyInterface, hwnd);
}
...
ULONG STDMETHODCALLTYPE Direct3DDevice8::AddRef() { return ProxyInterface->AddRef(); }
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::TestCooperativeLevel()
{
    ...
    auto reset = D3DERR_DEVICENOTRESET;
    auto e = D3DERR_DRIVERINTERNALERROR;
    HRESULT res = ProxyInterface->TestCooperativeLevel();
    return res;
}
uint STDMETHODCALLTYPE Direct3DDevice8::GetAvailableTextureMem() { return ProxyInterface->GetAvailableTextureMem(); }
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::GetDisplayMode(D3DDISPLAYMODE* pMode) { return ProxyInterface->GetDisplayMode(0, pMode); }
...
BOOL STDMETHODCALLTYPE Direct3DDevice8::ShowCursor(BOOL bShow) { return ProxyInterface->ShowCursor(bShow); }
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::Reset(D3DPRESENT_PARAMETERS8* pPresentationParameters)
{
    ...
    auto moved = D3DERR_DEVICEREMOVED;
    auto err = D3DERR_DRIVERINTERNALERROR;
    auto mem = D3DERR_OUTOFVIDEOMEMORY;
    auto lost = D3DERR_DEVICELOST;
    auto a = D3DERR_DEVICEHUNG;
    auto b = D3DERR_WRONGTEXTUREFORMAT;
    auto c = D3DERR_UNSUPPORTEDCOLOROPERATION;
    auto d = D3DERR_UNSUPPORTEDCOLORARG;
    auto e = D3DERR_UNSUPPORTEDALPHAOPERATION;
    auto f = D3DERR_UNSUPPORTEDALPHAARG;
    auto g = D3DERR_TOOMANYOPERATIONS;
    auto h = D3DERR_CONFLICTINGTEXTUREFILTER;
    auto i = D3DERR_UNSUPPORTEDFACTORVALUE;
    auto j = D3DERR_CONFLICTINGRENDERSTATE;
    auto k = D3DERR_UNSUPPORTEDTEXTUREFILTER;
    auto l = D3DERR_CONFLICTINGTEXTUREPALETTE;
    auto m = D3DERR_DRIVERINTERNALERROR;

    auto n = D3DERR_NOTFOUND;
    auto o = D3DERR_MOREDATA;
    auto p = D3DERR_DEVICELOST;
    auto q = D3DERR_DEVICENOTRESET;
    auto r = D3DERR_NOTAVAILABLE;
    auto s = D3DERR_OUTOFVIDEOMEMORY;
    auto t = D3DERR_INVALIDDEVICE;
    auto u = D3DERR_INVALIDCALL;
    auto v = D3DERR_DRIVERINVALIDCALL;
    auto w = D3DERR_WASSTILLDRAWING;
    HRESULT res = ProxyInterface->Reset(&PresentParams);
    return res;
}
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::GetRasterStatus(D3DRASTER_STATUS* pRasterStatus) { return ProxyInterface->GetRasterStatus(0, pRasterStatus); }
void STDMETHODCALLTYPE Direct3DDevice8::SetGammaRamp(DWORD Flags, const D3DGAMMARAMP* pRamp) { ProxyInterface->SetGammaRamp(0, Flags, pRamp); }
void STDMETHODCALLTYPE Direct3DDevice8::GetGammaRamp(D3DGAMMARAMP* pRamp) { ProxyInterface->GetGammaRamp(0, pRamp); }
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateTexture(uint Width, uint Height, uint Levels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool,
    ...
    *ppTexture = nullptr;
    ...
    if (Pool == D3DPOOL_DEFAULT)
    {
        ...
        if ((Usage & D3DUSAGE_DYNAMIC) == 0 &&
            SUCCEEDED(D3D->GetProxyInterface()->CheckDeviceFormat(
                CreationParams.AdapterOrdinal, CreationParams.DeviceType, D3DFMT_X8R8G8B8, D3DUSAGE_RENDERTARGET, D3DRTYPE_TEXTURE, Format)))
        {
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateVolumeTexture(uint Width, uint Height, uint Depth, uint Levels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool,
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateCubeTexture(uint EdgeLength, uint Levels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool,
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateRenderTarget(uint Width, uint Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample, BOOL Lockable,
                                                              Direct3DSurface8** ppSurface)
{
    ...
    *ppSurface = nullptr;
    ...
    if (MultiSample != D3DMULTISAMPLE_NONE)
    {
        ...
        D3D->GetProxyInterface()->CheckDeviceMultiSampleType(
            CreationParams.AdapterOrdinal, CreationParams.DeviceType, Format, FALSE, MultiSample, &QualityLevels);
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateDepthStencilSurface(uint Width, uint Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample,
                                                                     Direct3DSurface8** ppSurface)
{
    if (ppSurface == nullptr)
    {
    ...
    *ppSurface = nullptr;
    ...
    const HRESULT hr =
        ProxyInterface->CreateDepthStencilSurface(Width, Height, Format, MultiSample, QualityLevels, ZBufferDiscarding, &SurfaceInterface, nullptr);
    if (FAILED(hr))
    {
        return hr;
    }
                                                     ...
                                                     Direct3DSurface8* pDestinationSurface, const POINT* pDestPointsArray)
{
    ...
    for (uint i = 0; i < cRects; i++)
    {
        ...
        if (SourceDesc.Pool == D3DPOOL_MANAGED || DestinationDesc.Pool != D3DPOOL_DEFAULT)
        {
            ...
            {
                if (SUCCEEDED(D3DXLoadSurfaceFromSurface(pDestinationSurface->GetProxyInterface(),
                                                         nullptr,
                                                         &DestinationRect,
                                                         pSourceSurface->GetProxyInterface(),
                                                         nullptr,
                                                         &SourceRect,
                                                         D3DX_FILTER_NONE,
                                                         0)))
        ...
        else if (SourceDesc.Pool == D3DPOOL_DEFAULT)
        {
            hr = ProxyInterface->StretchRect(
                pSourceSurface->GetProxyInterface(), &SourceRect, pDestinationSurface->GetProxyInterface(), &DestinationRect, D3DTEXF_NONE);
        }
...
PBYTE ScreenData;
HRESULT STDMETHODCALLTYPE Direct3DDevice8::BeginScene() { return ProxyInterface->BeginScene(); }
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::EndScene()
{
    ...
    *((DWORD*)0x616840) = point.x;
    *((DWORD*)0x616844) = point.y;
    *((DWORD*)0x6107D4) = point.x;
    *((DWORD*)0x6107D8) = point.y;
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::GetViewport(D3DVIEWPORT8* pViewport) { return ProxyInterface->GetViewport(pViewport); }
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetMaterial(const D3DMATERIAL8* pMaterial) { return ProxyInterface->SetMaterial(pMaterial); }
HRESULT STDMETHODCALLTYPE Direct3DDevice8::GetMaterial(D3DMATERIAL8* pMaterial) { return ProxyInterface->GetMaterial(pMaterial); }
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetLight(DWORD Index, const D3DLIGHT8* pLight) { return ProxyInterface->SetLight(Index, pLight); }
HRESULT STDMETHODCALLTYPE Direct3DDevice8::GetLight(DWORD Index, D3DLIGHT8* pLight) { return ProxyInterface->GetLight(Index, pLight); }
HRESULT STDMETHODCALLTYPE Direct3DDevice8::LightEnable(DWORD Index, BOOL Enable) { return ProxyInterface->LightEnable(Index, Enable); }
HRESULT STDMETHODCALLTYPE Direct3DDevice8::GetLightEnable(DWORD Index, BOOL* pEnable) { return ProxyInterface->GetLightEnable(Index, pEnable); }
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetRenderState(D3DRENDERSTATETYPE State, DWORD Value)
{
    ...
    {
        case D3DRS_ZVISIBLE: return D3DERR_INVALIDCALL;
        case D3DRS_PATCHSEGMENTS:
        case D3DRS_LINEPATTERN:
        case D3DRS_SOFTWAREVERTEXPROCESSING: return D3D_OK;
        case D3DRS_EDGEANTIALIAS: return ProxyInterface->SetRenderState(D3DRS_ANTIALIASEDLINEENABLE, Value);
        ...
        default: return ProxyInterface->SetRenderState(State, Value);
    }
}
HRESULT STDMETHODCALLTYPE Direct3DDevice8::GetRenderState(D3DRENDERSTATETYPE State, DWORD* pValue)
{
    ...
    *pValue = 0;
    ...
    {
        case D3DRS_ZVISIBLE: return D3DERR_INVALIDCALL;
        case D3DRS_LINEPATTERN: *pValue = 0; return D3D_OK;
        case D3DRS_EDGEANTIALIAS: return ProxyInterface->GetRenderState(D3DRS_ANTIALIASEDLINEENABLE, pValue);
        case D3DRS_ZBIAS:
            ...
            *pValue = static_cast<DWORD>(*reinterpret_cast<const FLOAT*>(pValue) * -500000.0f);
            return hr;
        case D3DRS_SOFTWAREVERTEXPROCESSING: *pValue = ProxyInterface->GetSoftwareVertexProcessing(); return D3D_OK;
        case D3DRS_PATCHSEGMENTS: *pValue = 1; return D3D_OK;
        default: return ProxyInterface->GetRenderState(State, pValue);
    }
}
HRESULT STDMETHODCALLTYPE Direct3DDevice8::BeginStateBlock() { return ProxyInterface->BeginStateBlock(); }
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetClipStatus(const D3DCLIPSTATUS8* pClipStatus) { return ProxyInterface->SetClipStatus(pClipStatus); }
HRESULT STDMETHODCALLTYPE Direct3DDevice8::GetClipStatus(D3DCLIPSTATUS8* pClipStatus) { return ProxyInterface->GetClipStatus(pClipStatus); }
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetTexture(DWORD Stage, Direct3DBaseTexture8* pTexture)
{
    ...
    {
        case D3DRTYPE_TEXTURE: BaseTextureInterface = static_cast<Direct3DTexture8*>(pTexture)->GetProxyInterface(); break;
        case D3DRTYPE_VOLUMETEXTURE: BaseTextureInterface = static_cast<Direct3DVolumeTexture8*>(pTexture)->GetProxyInterface(); break;
        case D3DRTYPE_CUBETEXTURE: BaseTextureInterface = static_cast<Direct3DCubeTexture8*>(pTexture)->GetProxyInterface(); break;
        default: return D3DERR_INVALIDCALL;
    }
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::GetTextureStageState(DWORD Stage, D3DTEXTURESTAGESTATETYPE Type, DWORD* pValue)
{
    ...
    {
        case D3DTSS_ADDRESSU: return ProxyInterface->GetSamplerState(Stage, D3DSAMP_ADDRESSU, pValue);
        case D3DTSS_ADDRESSV: return ProxyInterface->GetSamplerState(Stage, D3DSAMP_ADDRESSV, pValue);
        case D3DTSS_ADDRESSW: return ProxyInterface->GetSamplerState(Stage, D3DSAMP_ADDRESSW, pValue);
        case D3DTSS_BORDERCOLOR: return ProxyInterface->GetSamplerState(Stage, D3DSAMP_BORDERCOLOR, pValue);
        case D3DTSS_MAGFILTER: return ProxyInterface->GetSamplerState(Stage, D3DSAMP_MAGFILTER, pValue);
        case D3DTSS_MINFILTER: return ProxyInterface->GetSamplerState(Stage, D3DSAMP_MINFILTER, pValue);
        case D3DTSS_MIPFILTER: return ProxyInterface->GetSamplerState(Stage, D3DSAMP_MIPFILTER, pValue);
        case D3DTSS_MIPMAPLODBIAS: return ProxyInterface->GetSamplerState(Stage, D3DSAMP_MIPMAPLODBIAS, pValue);
        case D3DTSS_MAXMIPLEVEL: return ProxyInterface->GetSamplerState(Stage, D3DSAMP_MAXMIPLEVEL, pValue);
        case D3DTSS_MAXANISOTROPY: return ProxyInterface->GetSamplerState(Stage, D3DSAMP_MAXANISOTROPY, pValue);
        default: return ProxyInterface->GetTextureStageState(Stage, Type, pValue);
    }
}
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetTextureStageState(DWORD Stage, D3DTEXTURESTAGESTATETYPE Type, DWORD Value)
{
    switch (static_cast<DWORD>(Type))
    {
        case D3DTSS_ADDRESSU: return ProxyInterface->SetSamplerState(Stage, D3DSAMP_ADDRESSU, Value);
        case D3DTSS_ADDRESSV: return ProxyInterface->SetSamplerState(Stage, D3DSAMP_ADDRESSV, Value);
        case D3DTSS_ADDRESSW: return ProxyInterface->SetSamplerState(Stage, D3DSAMP_ADDRESSW, Value);
        case D3DTSS_BORDERCOLOR: return ProxyInterface->SetSamplerState(Stage, D3DSAMP_BORDERCOLOR, Value);
        case D3DTSS_MAGFILTER: return ProxyInterface->SetSamplerState(Stage, D3DSAMP_MAGFILTER, Value);
        case D3DTSS_MINFILTER: return ProxyInterface->SetSamplerState(Stage, D3DSAMP_MINFILTER, Value);
        case D3DTSS_MIPFILTER: return ProxyInterface->SetSamplerState(Stage, D3DSAMP_MIPFILTER, Value);
        case D3DTSS_MIPMAPLODBIAS: return ProxyInterface->SetSamplerState(Stage, D3DSAMP_MIPMAPLODBIAS, Value);
        case D3DTSS_MAXMIPLEVEL: return ProxyInterface->SetSamplerState(Stage, D3DSAMP_MAXMIPLEVEL, Value);
        case D3DTSS_MAXANISOTROPY: return ProxyInterface->SetSamplerState(Stage, D3DSAMP_MAXANISOTROPY, Value);
        default: return ProxyInterface->SetTextureStageState(Stage, Type, Value);
    }
}
HRESULT STDMETHODCALLTYPE Direct3DDevice8::ValidateDevice(DWORD* pNumPasses) { return ProxyInterface->ValidateDevice(pNumPasses); }
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::DrawIndexedPrimitive(D3DPRIMITIVETYPE PrimitiveType, uint MinIndex, uint NumVertices, uint StartIndex,
                                                                uint PrimitiveCount)
{
    ApplyClipPlanes();
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::DrawPrimitiveUP(D3DPRIMITIVETYPE PrimitiveType, uint PrimitiveCount, const void* pVertexStreamZeroData,
                                                           uint VertexStreamZeroStride)
{
    ApplyClipPlanes();
    return ProxyInterface->DrawPrimitiveUP(PrimitiveType, PrimitiveCount, pVertexStreamZeroData, VertexStreamZeroStride);
}
HRESULT STDMETHODCALLTYPE Direct3DDevice8::DrawIndexedPrimitiveUP(D3DPRIMITIVETYPE PrimitiveType, uint MinVertexIndex, uint NumVertexIndices,
                                                                  ...
                                                                  const void* pVertexStreamZeroData, uint VertexStreamZeroStride)
{
    ...
    return ProxyInterface->DrawIndexedPrimitiveUP(
        PrimitiveType, MinVertexIndex, NumVertexIndices, PrimitiveCount, pIndexData, IndexDataFormat, pVertexStreamZeroData, VertexStreamZeroStride);
}
HRESULT STDMETHODCALLTYPE Direct3DDevice8::ProcessVertices(uint SrcStartIndex, uint DestIndex, uint VertexCount, Direct3DVertexBuffer8* pDestBuffer,
                                                           DWORD Flags)
{
    ...
    *pHandle = 0;
    ...
    if (pFunction != nullptr)
    {
        ...
        for (uint k = 0; k < ElementIndex; k++)
        {
            ...
            {
                case D3DDECLUSAGE_POSITION: DeclCode += "dcl_position"; break;
                case D3DDECLUSAGE_BLENDWEIGHT: DeclCode += "dcl_blendweight"; break;
                case D3DDECLUSAGE_BLENDINDICES: DeclCode += "dcl_blendindices"; break;
                case D3DDECLUSAGE_NORMAL: DeclCode += "dcl_normal"; break;
                case D3DDECLUSAGE_PSIZE: DeclCode += "dcl_psize"; break;
                case D3DDECLUSAGE_COLOR: DeclCode += "dcl_color"; break;
                case D3DDECLUSAGE_TEXCOORD: DeclCode += "dcl_texcoord"; break;
            }
...
#pragma region Fill registers with default value
        ...
        size_t InstructionCount = InstructionPosition > 2 && InstructionPosition < SourceCode.size()
                                      ? strtoul(SourceCode.substr(InstructionPosition - 4, 4).c_str(), nullptr, 10)
                                      : 0;
...
#pragma endregion
        ...
        SourceCode =
            std::regex_replace(SourceCode, std::regex("(add|sub|mul|min|max) (oFog|oPts), ([cr][0-9]+), (.+)\\n"), "$1 $2, $3.x /* added swizzle */, $4\n");
        SourceCode =
            std::regex_replace(SourceCode, std::regex("(add|sub|mul|min|max) (oFog|oPts), (.+), ([cr][0-9]+)\\n"), "$1 $2, $3, $4.x /* added swizzle */\n");
        SourceCode = std::regex_replace(
            SourceCode, std::regex("(mov|mad) (oFog|oPts)(.*), (-?)([crv][0-9]+(?![\\.0-9]))"), "$1 $2$3, $4$5.x /* select single component */");
        ...
        if (std::regex_search(SourceCode, std::regex("m.x.")))
        {
            ...
            for (r = 0; r < 12; r++)
            {
                if (SourceCode.find("r" + std::to_string(r)) == std::string::npos)
                {
                    break;
                }
            }
                ...
                while (std::regex_search(SourceCode, std::regex(reg)))
                {
                    ...
                    if (r < 12 && InstructionCount < 128)
                    {
                        ...
                        SourceCode = std::regex_replace(SourceCode,
                                                        std::regex(reg),
                                                        "mov r" + std::to_string(r) + ", $2 /* added line */\n    $1 $2, $4r" + std::to_string(r) +
                                                            "$5 /* changed $3 to r" + std::to_string(r) + " */",
        ...
        if (std::regex_search(SourceCode, std::regex("    ([a-z2-4]*) oPos\\.")) && !std::regex_search(SourceCode, std::regex("    ([a-z2-4]*) oPos,")))
        {
            ...
            {
                SourceCode = std::regex_replace(
                    SourceCode, std::regex("    ([a-z2-4]*) (oPos\\.[x|y|z|w]*,) ([^\\n]*)\\n"), "    $1 oPos, $3 /* removed oPos swizzles */\n");
            }
        }
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreatePixelShader(const DWORD* pFunction, DWORD* pHandle)
{
    ...
    *pHandle = 0;
    ...
    size_t ArithmeticCount =
        ArithmeticPosition > 2 && ArithmeticPosition < SourceCode.size() ? strtoul(SourceCode.substr(ArithmeticPosition - 2, 2).c_str(), nullptr, 10) : 0;
    ...
    SourceCode = std::regex_replace(
        SourceCode,
        std::regex(
            "(add)([_satxd248]*) (r[0-9][\\.wxyz]*), ((1-|)[crtv][0-9][\\.wxyz_abdis2]*), (-)(c[0-9][\\.wxyz]*)(_bx2|_bias|_x2|_d[zbwa]|)(?![_\\.wxyz])"),
    ...
    while (std::regex_search(SourceCode, std::regex("-c[0-9]|c[0-9][\\.wxyz]*_")) && ArithmeticCount < 8)
    {
        ...
        SourceCode = std::regex_replace(
            SourceCode,
            std::regex("    (...)(_[_satxd248]*|) (r[0-9])([\\.wxyz]*), (1?-?[crtv][0-9][\\.wxyz_abdis2]*, )?(1?-?[crtv][0-9][\\.wxyz_abdis2]*, "
                       ")?(1?-?[crtv][0-9][\\.wxyz_abdis2]*, "
                       ")?((1?-)(c[0-9])([\\.wxyz]*)(_bx2|_bias|_x2|_d[zbwa]|)|(1?-?)(c[0-9])([\\.wxyz]*)(_bx2|_bias|_x2|_d[zbwa]))(?![_\\.wxyz])"),
        ...
        if (SourceSize == SourceCode.size())
        {
            SourceCode = std::regex_replace(
                ...
                std::regex("(    .*\\n)  \\+ (...)(_[_satxd248]*|) (r[0-9])([\\.wxyz]*), (1?-?[crtv][0-9][\\.wxyz_abdis2]*, "
                           ")?(1?-?[crtv][0-9][\\.wxyz_abdis2]*, )?(1?-?[crtv][0-9][\\.wxyz_abdis2]*, "
                           ")?((1?-)(c[0-9])([\\.wxyz]*)(_bx2|_bias|_x2|_d[zbwa]|)|(1?-?)(c[0-9])([\\.wxyz]*)(_bx2|_bias|_x2|_d[zbwa]))(?![_\\.wxyz])"),
                ...
                if (std::regex_search(NewLine, std::regex("-c[0-9]|c[0-9][\\.wxyz]*_")))
                {
                    for (size_t j = 0; j < 6; j++)
                    {
                        ...
                        if (NewSourceCode.find(reg) == std::string::npos)
                        {
                            ...
                            if (std::regex_search(SourceCode14.substr(LinePosition + NewLine.length(), SourceCode14.length()),
                                                  std::regex("-" + constReg + "|" + constReg + "[\\.wxyz]*_")))
                            {
                                // Find an unused register
                                while (j < 6 && (NewSourceCode.find("r" + std::to_string(j)) != std::string::npos ||
                ...
                if (std::regex_search(NewLine, std::regex("t[0-9]")))
                {
                    ...
                    if (SourceCode14.find("t" + texNum, start) == std::string::npos)
                    {
                        ...
                        if (!RegisterUsed[(Num < 6) ? Num : 6])
                        {
                            ...
                            if (NewSourceCode.find("r" + destRegNum) == std::string::npos || SourceCode14.find("r" + destRegNum, start) == std::string::npos)
                            {
                                ...
                                const std::string tempSourceCode =
                                    std::regex_replace(SourceCode14.substr(start, SourceCode14.length()), std::regex("r" + destRegNum), "r" + texNum);
    ...
    SourceCode = std::regex_replace(SourceCode,
                                    std::regex("(mad)([_satxd248]*) (r[0-9][\\.wxyz]*), (1?-?[crtv][0-9][\\.wxyz_abdis2]*), "
                                               "(1?-?[crtv][0-9][\\.wxyz_abdis2]*), (-)(c[0-9][\\.wxyz]*)(_bx2|_bias|_x2|_d[zbwa]|)(?![_\\.wxyz])"),
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::DeletePatch(uint Handle) { return ProxyInterface->DeletePatch(Handle); }
