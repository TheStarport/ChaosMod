...
namespace Utils
{
    ...
    namespace Memory
    {
        ...
        void WriteProcMem(DWORD addr, void* pMem, int iSize);
        void NopAddress(unsigned int address, unsigned int pSize);
        void ReadProcMem(void* pAddress, void* pMem, int iSize);
        inline void ReadProcMem(DWORD pAddress, void* pMem, int iSize) { ReadProcMem(reinterpret_cast<void*>(pAddress), pMem, iSize); }
    ...
    class CommandLineParser
    {
        public:
            CommandLineParser();
            [[nodiscard]]
            std::wstring GetCmdOption(const std::wstring& option) const;
            [[nodiscard]]
            bool CmdOptionExists(const std::wstring& option) const;
